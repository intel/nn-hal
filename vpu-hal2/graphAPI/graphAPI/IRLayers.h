...
inline OutputPort Reshape(const TensorDims &newDims, const OutputPort &src)
{
    ...
    //auto type = src->creatorLayer.lock()->type;

    //if((src->getCreatorLayer())->type == "Reshape") // fuse reshapes
    //if(strncasecmp(type.c_str(), "Reshape", type.size()) == 0)
/*
...
*/
    std::string layerName = "Reshape";
    //auto op = output(Generic("Reshape", src));
    auto op = output(Generic(layerName, src));
    op->setDims(newDims);
    //FIX ME : HACK
    //TensorDims dims = {1, newDims[0], 1, 1};
    //op->setDims(dims);
