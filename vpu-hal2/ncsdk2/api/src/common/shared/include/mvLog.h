...
//unsigned int __attribute__ ((weak)) MVLOGLEVEL(MVLOG_UNIT_NAME) = MVLOG_ERROR; //aks

static unsigned int MVLOGLEVEL(default) = MVLOG_INFO;
//static unsigned int MVLOGLEVEL(default) = MVLOG_ERROR; //aks
                     ...
                     const char * format, ...)
{
    ...
    return 0;
#endif
    const char headerFormat[] = "%s [%10llu] %s:%d\t";
#ifdef __RTEMS__
