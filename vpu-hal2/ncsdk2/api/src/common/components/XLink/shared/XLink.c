...
XLinkGlobalHandler_t* glHandler; //TODO EMAN need to either protect this with semaphor
...
int handleIncomingEvent(xLinkEvent_t* event){
    ...
    case USB_WRITE_REQ:
        ...
        stream->localFillLevel += ALIGN_UP(event->header.size, __CACHE_LINE_SIZE);
        mvLog(MVLOG_DEBUG,"Got write, current local fill level is %ld out of %ld %ld\n", stream->localFillLevel, stream->readSize, stream->writeSize);
 ...
 int dispatcherEventReceive(xLinkEvent_t* event){
    ...
    {
        //TODO: Historically this check comes from a bug in the myriad USB stack. Shouldn't be the case anymore
...
int releasePacketFromStream(streamDesc_t* stream, uint32_t* releasedSize)
{
    ...
    stream->localFillLevel -= ALIGN_UP((int32_t)currPack->length, __CACHE_LINE_SIZE);
    mvLog(MVLOG_DEBUG,"Got release, current local fill level is %ld out of %ld %ld\n", stream->localFillLevel, stream->readSize, stream->writeSize);

    deallocateData(currPack->data, ALIGN_UP((int32_t)currPack->length, __CACHE_LINE_SIZE), __CACHE_LINE_SIZE);
    CIRCULAR_INCREMENT(stream->firstPacket, USB_LINK_MAX_PACKETS_PER_STREAM);
    stream->blockedPackets--;
    *releasedSize = ALIGN_UP(currPack->length, __CACHE_LINE_SIZE);
    return 0;
}
...
int dispatcherLocalEventGetResponse(xLinkEvent_t* event, xLinkEvent_t* response)
{
    ...
    case USB_WRITE_REQ:
        ...
        if(!isStreamSpaceEnoughFor(stream, ALIGN_UP(event->header.size, __CACHE_LINE_SIZE))){
            ...
            stream->remoteFillLevel += ALIGN_UP(event->header.size, __CACHE_LINE_SIZE);
            stream->remoteFillPacketLevel++;

            mvLog(MVLOG_DEBUG,"Got local write remote fill level %ld out of %ld %ld\n", stream->remoteFillLevel, stream->writeSize, stream->readSize);
        }
        releaseStream(stream);
        break;
    case USB_READ_REQ:
...
int dispatcherRemoteEventGetResponse(xLinkEvent_t* event, xLinkEvent_t* response)
{
    ...
    switch (event->header.type)
    {
        ...
        case USB_READ_REL_REQ:
            ...
            mvLog(MVLOG_DEBUG,"Got remote release, remote fill level %ld out of %ld %ld\n",
                  stream->remoteFillLevel, stream->writeSize, stream->readSize);
            releaseStream(stream);
...
{
    USBLinkPlatformResetRemote(fd);//TODO EMAN
}
