...
typedef enum {
    ...
    NC_COMMUNICATION_NOT_OPEN = -14,
    NC_COMMUNICATION_FAIL = -15,
} ncStatus_t;
...
typedef enum {
    ...
    NC_RO_GRAPH_AUX_BUFFER_SIZE = 1010, // returns aux buffer size, size_t
...
typedef enum {
    NC_RO_DEVICE_THERMAL_STATS = 2000,  // Return throttling flag at the [0] and temperatures from [1], float *, not for general use
    ...
    NC_RO_DEVICE_ID = 2015, // returns device id
    NC_RO_DEVICE_PLATFORM = 2016, // returns device platform (MyriadX, Myriad2)
} ncDeviceOptionsClass0_t;
...
ncStatus_t ncDeviceReset(struct deviceHandle_t *deviceHandle);
ncStatus_t ncDeviceResetAll();

ncStatus_t ncQueryOutputBufferSize(const void *graphFile, unsigned int numElem, void* output_data_size);
