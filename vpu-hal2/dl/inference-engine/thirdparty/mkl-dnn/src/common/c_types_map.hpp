...
namespace primitive_kind {
    ...
    const primitive_kind_t simpler_nms = mkldnn_simpler_nms;
}
...
namespace query {
    ...
    const query_t simpler_nms_d = mkldnn_query_simpler_nms_d;
...
using simpler_nms_desc_t = mkldnn_simpler_nms_desc_t;
...
struct op_desc_t {
    union {
        ...
        simpler_nms_desc_t simpler_nms;
    };
...
#   define DECL_CTOR_AND_CONVERTERS(c_type, name) \
    ...
    DECL_CTOR_AND_CONVERTERS(simpler_nms_desc_t, simpler_nms);
