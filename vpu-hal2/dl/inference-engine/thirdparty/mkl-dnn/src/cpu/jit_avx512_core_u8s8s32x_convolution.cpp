...
bool jit_avx512_core_u8s8s32x_conv_fwd_ker_t::post_ops_ok(
    ...
    case 0: return true;
    case 1: return true
                && implication(c.with_eltwise, p.contain(sum, 0))
                && implication(!c.with_eltwise, is_eltwise(0) || p.contain(sum, 0));
    case 2: return true
                && implication(c.with_eltwise, p.contain(sum, 0) && is_eltwise(1))
                && implication(!c.with_eltwise, false
                        || (p.contain(sum, 0) && is_eltwise(1))
                        || (p.contain(sum, 1) && is_eltwise(0)));
    case 3: return true
                && c.with_eltwise == false
                && (is_eltwise(0) && p.contain(sum, 1) && is_eltwise(2));
