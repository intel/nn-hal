...
void jit_avx512_common_conv_fwd_kernel::store_output(int ur_w)
{
    ...
    if (jcp.with_eltwise) {
        ...
        if (jcp.eltwise_alpha == 0 || jcp.ver == ver_4vnni) {
        ...
        if (this->jcp.eltwise_alg == mkldnn_eltwise_relu) {
            ...
            mov(reg_table, jit_avx512_common_conv_fwd_kernel::l_table);
            const unsigned char _cmp_gt_os = 14;

            for (int k = 0; k < jcp.nb_oc_blocking; k++) {
                for (int j = 0; j < ur_w; j++) {
                    Zmm zmm_reg_out = zmm_out(j, k);

                    uni_vmovups(zmm_src, zmm_reg_out);
                    simd_expf(zmm_reg_out);
                    uni_vsubps(zmm_reg_out, zmm_reg_out, ptr[reg_table + 0 * 64]);
                    uni_vmulps(zmm_reg_out, zmm_reg_out, zmm_relu_ns);

                    Opmask kmask = Opmask(1);
                    vpxord(zmm_zero, zmm_zero, zmm_zero);
                    vcmp(kmask, zmm_src, zmm_zero, _cmp_gt_os);

                    vblendmps(zmm_reg_out | kmask, zmm_reg_out, zmm_src);
                }
            }
        }
    }
...
//    const int regs = 28;
    // FIXME (ichuraev): batch > 1 crashed on PropertyTest.MKLDNN_ICV_LPRNet_fp32 with regs = 28 jcp.ur_w
    const int regs = 27;
    ...
    if (jcp.with_eltwise && (jcp.eltwise_alg == mkldnn_eltwise_elu) &&
         (jcp.nb_oc_blocking * jcp.ur_w) > 28) // we need at least 4 vector registers to compute ELU
    {
        return status::unimplemented;
    }
...
void jit_avx512_common_conv_fwd_kernel::simd_expf(const Zmm &zmm_src) {
    uni_vminps(zmm_src, zmm_src, ptr[reg_table + 10 * 64]);
    uni_vmaxps(zmm_src, zmm_src, ptr[reg_table + 11 * 64]);
    uni_vmovups(zmm_aux0, zmm_src);
    //calculate exp(x)
    // fx = x * log2ef + 0.5
    uni_vmulps(zmm_src, zmm_src, ptr[reg_table + 2 * 64]);
    uni_vaddps(zmm_src, zmm_src, ptr[reg_table + 1 * 64]);

    // tmp = floorf(fx)
    vcvtps2dq(zmm_aux1 | T_rd_sae, zmm_src);
    vcvtdq2ps(zmm_aux1, zmm_aux1);

    unsigned char _cmp_gt_os = 14;
    vcmp(k_mask_tmp, zmm_aux1, zmm_src, _cmp_gt_os);
    vmovups(zmm_aux2 | k_mask_tmp | T_z, zword[reg_table + 0 * 64]);

    // fx = fx - 1 (if there are fraction bits)
    uni_vsubps(zmm_aux1, zmm_aux1, zmm_aux2);

    //keep fx for further computations
    uni_vmovups(zmm_src, zmm_aux1); //zmm_src = fx

    //x = x - fx * ln2
    uni_vfnmadd231ps(zmm_aux0, zmm_aux1, ptr[reg_table + 3 * 64]);

    // compute 2^n
    uni_vcvtps2dq(zmm_aux1, zmm_src);
    uni_vpaddd(zmm_aux1, zmm_aux1, ptr[reg_table + 4 * 64]);
    uni_vpslld(zmm_aux1, zmm_aux1, 23); //Vmm(6) = 2^-fx

    // y = p5
    uni_vmovups(zmm_src, ptr[reg_table + 9 * 64]);
    // y = y * x + p4
    uni_vfmadd213ps(zmm_src, zmm_aux0, ptr[reg_table + 8 * 64]);
    // y = y * x + p3
    uni_vfmadd213ps(zmm_src, zmm_aux0, ptr[reg_table + 7 * 64]);
    // y = y * x + p2
    uni_vfmadd213ps(zmm_src, zmm_aux0, ptr[reg_table + 6 * 64]);
    // y = y * x + p1
    uni_vfmadd213ps(zmm_src, zmm_aux0, ptr[reg_table + 0 * 64]);
    // y = y * x + p0
    uni_vfmadd213ps(zmm_src, zmm_aux0, ptr[reg_table + 5 * 64]);  //exp(q)
    // y = y * 2^n
    uni_vmulps(zmm_src, zmm_src, zmm_aux1);
}

void jit_avx512_common_conv_fwd_kernel::prepare_table() {
