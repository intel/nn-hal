        ...
        int oc_blocks, char oc_blocks_tag)
{
    ...
    if (this->jcp.with_eltwise) {
        ...
        if (this->jcp.eltwise_alg == mkldnn_eltwise_relu) {
            ...
            mov(reg_table, jit_sse42_conv_fwd_kernel_f32::l_table);
            ...
            for (int ii = 0; ii < oc_blocks; ii++) {
                for (int jj = 0; jj < ur_w; jj++) {
                    ...
                    // compute exponent
                    movups(xmm_src, reg_out);
                    simd_expf(reg_out);

                    // alpha * (exp(x) - 1)
                    subps(reg_out, ptr[reg_table + 0 * 16]);
                    mulps(reg_out, xmm_relu_ns);
                    ...
                    blendvps(xmm_src, reg_out);

                    movups(xword[reg_output + sizeof(float) * o_off], xmm_src);
                }
            }
        }
...
void jit_sse42_conv_fwd_kernel_f32::simd_expf(const Xmm &xmm_src) {
...
void jit_sse42_conv_fwd_kernel_f32::prepare_table() {
