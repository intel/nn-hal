...
void im2col(
    ...
    auto im2col_common = [&](const float *im, float *col) {
        ...
        int col_h = (ih + 2 * pad_h - dil_kernel_h) / stride_h + 1;
        int col_w = (iw + 2 * pad_w - dil_kernel_w) / stride_w + 1;
        ...
        for (int c = 0; c < col_c; ++c) {
            ...
            const int im_h_start = h_offset * dilate_h - pad_h;
            const int im_w_start = w_offset * dilate_w - pad_w;
...
void col2im(
    ...
    int col_h = (ih + 2 * pad_h - dil_patch_h) / stride_h + 1;
    int col_w = (iw + 2 * pad_w - dil_patch_w) / stride_w + 1;
