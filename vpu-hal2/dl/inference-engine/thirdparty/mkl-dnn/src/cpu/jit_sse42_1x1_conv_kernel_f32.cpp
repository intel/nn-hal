    ...
    auto store = [=]() {
        ...
        if (jcp.with_eltwise) {
            ...
            if (this->jcp.eltwise_alg == mkldnn_eltwise_relu) {
                ...
                mov(reg_table, jit_sse42_1x1_conv_kernel_f32::l_table);

                for (int j = 0; j < ur; ++j) {
                    for (int i = 0; i < load_loop_blk; ++i) {
                        const unsigned char _cmp_gt_os = 6;
                        movups(xmm_src, reg_accum(i, j, 0));
                        simd_expf(reg_accum(i, j, 0));
                        subps(reg_accum(i, j, 0), ptr[reg_table + 0 * 16]);
                        mulps(reg_accum(i, j, 0), xmm_relu_ns);
                        pxor(xmm_mask, xmm_mask);
                        cmpps(xmm_mask, xmm_src, _cmp_gt_os);
                        blendvps(xmm_src, reg_accum(i, j, 0));
                        movups(output_ptr(i, j, 0), xmm_src);
                        movups(xmm_src, reg_accum(i, j, 1));
                        simd_expf(reg_accum(i, j, 1));
                        subps(reg_accum(i, j, 1), ptr[reg_table + 0 * 16]);
                        mulps(reg_accum(i, j, 1), xmm_relu_ns);
                        pxor(xmm_mask, xmm_mask);
                        cmpps(xmm_mask, xmm_src, _cmp_gt_os);
                        blendvps(xmm_src, reg_accum(i, j, 1));
                        movups(output_ptr(i, j, 1), xmm_src);
                    }
                }
            }
...
void jit_sse42_1x1_conv_kernel_f32::simd_expf(const Xmm &xmm_src) {
    uni_vminps(xmm_src, xmm_src, ptr[reg_table + 10 * 16]);
    uni_vmaxps(xmm_src, xmm_src, ptr[reg_table + 11 * 16]);
    uni_vmovups(xmm_aux0, xmm_src);
    //calculate exp(x)
    // fx = x * log2ef + 0.5
    uni_vmulps(xmm_src, xmm_src, ptr[reg_table + 2 * 16]);
    uni_vaddps(xmm_src, xmm_src, ptr[reg_table + 1 * 16]);

    // tmp = floorf(fx)
    uni_vroundps(xmm_aux1, xmm_src, _op_floor);
    //keep fx for further computations
    uni_vmovups(xmm_src, xmm_aux1); //xmm_src = fx

    //x = x - fx * ln2
    uni_vfnmadd231ps(xmm_aux0, xmm_aux1, ptr[reg_table + 3 * 16]);

    // compute 2^n
    uni_vcvtps2dq(xmm_aux1, xmm_src);
    uni_vpaddd(xmm_aux1, xmm_aux1, ptr[reg_table + 4 * 16]);
    uni_vpslld(xmm_aux1, xmm_aux1, 23); //Vmm(6) = 2^-fx

    // y = p5
    uni_vmovups(xmm_src, ptr[reg_table + 9 * 16]);
    // y = y * x + p4
    uni_vfmadd213ps(xmm_src, xmm_aux0, ptr[reg_table + 8 * 16]);
    // y = y * x + p3
    uni_vfmadd213ps(xmm_src, xmm_aux0, ptr[reg_table + 7 * 16]);
    // y = y * x + p2
    uni_vfmadd213ps(xmm_src, xmm_aux0, ptr[reg_table + 6 * 16]);
    // y = y * x + p1
    uni_vfmadd213ps(xmm_src, xmm_aux0, ptr[reg_table + 0 * 16]);
    // y = y * x + p0
    uni_vfmadd213ps(xmm_src, xmm_aux0, ptr[reg_table + 5 * 16]);  //exp(q)
    // y = y * 2^n
    uni_vmulps(xmm_src, xmm_src, xmm_aux1);
}

void jit_sse42_1x1_conv_kernel_f32::prepare_table() {
