        ...
        int oc_blocks, char oc_blocks_tag)
{
    ...
    if (this->jcp.with_eltwise) {
        ...
        if (this->jcp.eltwise_alg == mkldnn_eltwise_relu) {
            ...
            mov(reg_table, jit_avx2_conv_fwd_kernel_f32::l_table);
            ...
            for (int ii = 0; ii < oc_blocks; ii++) {
                for (int jj = 0; jj < ur_w; jj++) {
                    ...
                    // combine with mask
                    vxorps(yzero, yzero, yzero);
                    uni_vcmpgtps(ymm_mask, ymm_src, yzero);
...
void jit_avx2_conv_fwd_kernel_f32::simd_expf(const Ymm &ymm_src) {
...
void jit_avx2_conv_fwd_kernel_f32::prepare_table() {
        ...
        int r_overflow, const char *kh_label)
{
    ...
    for (int ii = 0; ii < nb_ic_block; ii++)
        for (int jj = 0; jj < ur_w; jj++)
            vmovups(Ymm(ur_w * ii + jj),
                    ptr[reg_dsrc
                    + sizeof(float) * (ii * ih * iw + jj)  * ic_block]);
            ...
            for (int ofm2 = 0; ofm2 < jcp.oc_block; ofm2++) {
                ...
                for (int ii = 0; ii  < nb_ic_block; ii++) {
                    int aux_kernel_offset
                        = ii * kh * kw * jcp.ic_block * jcp.oc_block
