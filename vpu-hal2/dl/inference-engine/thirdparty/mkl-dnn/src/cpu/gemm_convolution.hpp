    ...
    protected:
        ...
        virtual bool is_gemm_conv_format() const {
            ...
            case 0: // no post_ops
                break;
            case 1:
                ok = ok && // sum OR eltwise
                        (po.entry_[0].is_relu(true, false) || po.entry_[0].is_elu(true, false) || po.contain(sum, 0));
                break;
            case 2:
                ok = ok && // sum->eltwise
                        (po.contain(sum, 0) && (po.entry_[1].is_relu(true, false) || po.entry_[1].is_elu(true, false)));
                break;
            default: ok = false;
            }
            return ok;
