...
status_t jit_uni_pool_kernel_f32<isa>::init_conf(jit_pool_conf_t &jpp,
    ...
    jpp.b_pad = pd.padding[1][0];
    jpp.r_pad = pd.padding[1][1];
...
inline void jit_uni_pool_kernel_f32<isa>::avg_step(int ur_w, int pad_l,
        int pad_r, const char* kh_label, int pad_r_logic) {
...
void jit_uni_pool_kernel_f32<isa>::generate() {
    ...
    if (l_pad > 0) {
        n_oi--;
        if (n_oi < 0 && r_pad1 > 0) {
            step(ur_w, l_pad, r_pad1, ".kh_loop_oimain_padwl", r_pad1_log);
        } else  {
            step(ur_w, l_pad, 0, ".kh_loop_oimain_padwl", 0);
        }

        if (isa == sse42) {
            if (n_oi < 0 && r_pad1 > 0) {
                step_high_half(ur_w, l_pad, r_pad1,
                    ".kh_loop_oimain_padwl_high_half", r_pad1_log);
            } else  {
                step_high_half(ur_w, l_pad, 0,
                    ".kh_loop_oimain_padwl_high_half", 0);
            }
        }
    ...
    if (n_oi > 0) {
        L(".ow_loop"); {
            step(ur_w, 0, 0, ".kh_loop_oimain", 0);

            if (isa == sse42) {
                step_high_half(ur_w, 0, 0, ".kh_loop_oimain_high_half", 0);
            }
    ...
    if (r_pad1 > 0 && n_oi >= 0) {
        step(ur_w, 0, r_pad1, ".kh_loop_oimain_padwr", r_pad1_log);

        if (isa == sse42) {
            step_high_half(ur_w, 0, r_pad1, ".kh_loop_oimain_padwr_high_half", r_pad1_log);
        }
    ...
    if (ur_w_tail != 0) {
        step(ur_w_tail, 0, r_pad, ".kh_loop_oitail", r_pad_log);

        if (isa == sse42) {
            step_high_half(ur_w_tail, 0, r_pad, ".kh_loop_oitail_high_half", r_pad_log);
        }
    }
