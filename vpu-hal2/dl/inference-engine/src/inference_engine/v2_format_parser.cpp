...
inline void ParseSegment(LayerParseParameters& prms, const pugi::xml_node &blob) {
    ...
    segment.size = size;
    segment.start = GetIntAttr(blob, "offset", 0);
...
InferenceEngine::CNNLayer* V2FormatParser::CreateLayer(pugi::xml_node& node,
                                                       LayerParseParameters& layerParsePrms) const {
    static auto genericCreator = new V2LayerCreator<GenericLayer>("");
