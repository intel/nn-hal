...
public:
    MKLDNNBatchNormalizationNode(Type type, const std::string &name);
    explicit MKLDNNBatchNormalizationNode(InferenceEngine::CNNLayerPtr layer);
    virtual ~MKLDNNBatchNormalizationNode() {}

    void createDescriptor(mkldnn::memory::data_type inputDataType, mkldnn::memory::data_type outputDataType) override;
    void createPrimitive() override;
    bool created() override;
    bool fusedWithScale() const {return fusedWith.size() == 1 && fusedWith[0]->getType() == ScaleShift;}
private:
