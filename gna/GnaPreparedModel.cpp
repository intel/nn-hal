...
IRBlob::Ptr GnaPreparedModel::GetConstWeightsOperandAsTensor(uint32_t index, const Model &model) {
    ...
    if (op.type == OperandType::TENSOR_FLOAT32 || op.type == OperandType::FLOAT32) {
        if (buf == nullptr) ALOGE("TENSOR_FLOAT32 buf is NULL !!!!!!!!!!!!!!!");
        ...
        if (buf == nullptr) ALOGE("TENSOR_INT32 buf is NULL !!!!!!!!!!!!!!!");
        ...
        ALOGE("Do not support const tensors of type ", op.type);
        nnAssert(false);
    }
    ALOGV("Exiting %s", __func__);
    return nullptr;
...
IRBlob::Ptr GnaPreparedModel::GetConstOperandAsTensor(int operand_index, int operation_idx,
    ...
    ALOGI("GnaPreparedModel:: Operand: index: %d, len: %d, buf: %p", operand_index, len, buf);
    if (op.type == OperandType::TENSOR_FLOAT32 || op.type == OperandType::FLOAT32) {
        vec<unsigned int> order;
        Layout layout;
        if (op.dimensions.size() == 4) {
...
Blob::Ptr GnaPreparedModel::GetInOutOperandAsBlob(RunTimeOperandInfo &op, const uint8_t *buf,
    ...
    if (op.type == OperandType::TENSOR_FLOAT32 || op.type == OperandType::FLOAT32) {
        if (op.lifetime == OperandLifeTime::MODEL_INPUT) {
            ...
            if (buf == nullptr) ALOGE("MODEL_INPUT buf is NULL !!!!!!!!!!!!!!!");
            ...
            if (buf == nullptr) ALOGE("MODEL_OUTPUT buf is NULL !!!!!!!!!!!!!!!");
