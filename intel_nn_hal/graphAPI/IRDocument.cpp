...
#endif
    std::cout << "doc addOutput datadims size " << docdatadims.size()
              << std::endl;
    // std::cout << "docdatadims[0] "<<docdatadims[0]<< "docdatadims[1]"
    // <<docdatadims[1]<< std::endl;

    _outputData[data->name] = data;
  }
};

IRDocument::IRDocument(const std::string &cs) : _name(cs) {
  network = new InternalNetworkImpl(cs);
}
...
void IRDocument::optimize() {
  for (auto it = _layers.begin(); it != _layers.end();) {
    ...
    } /*else if (l->type == "Convolution"){ //convolution layer fused with
    Activatio layer
      auto convLayer = l;
      ++it;
      if (it == _layers.end()) return;
      auto fusedLayer = *it;
      if (fusedLayer->type == "ReLU" || fusedLayer->type == "ReLU6" ||
    fusedLayer->type == "Clamp") {
          convLayer->_fusedWith = fusedLayer;
          #ifdef NNLOG
          ALOGI("layer = %s fused with %s", convLayer->name.c_str(),
    fusedLayer->name.c_str());
          #endif
      }
    }*/
    else {
...
#include <log/log.h>
  ...
  void IRDocument::saveBlobToIR(
    ...
    */
    ...
    // if(IRBuilder::g_layer_precision == Precision::FP16)  //fix me: handle
    // float case
  ...
  void IRDocument::save(std::ostream & xml_os, std::ostream & bin_os) {
    ...
    int icnt = 0;
    for (auto &kvp :
         netInputs) { // todo: consider adding input layer to actual network...
...
// ALOGI("save cnn_layer name = %s", cnn_layer->name.c_str());
    ...
    for (auto &kvp : _layers) {
      ...
      for (auto inputData : kvp->insData) {
        ...
        edge.from.lid =
            b ? 0 : inputData.lock()->creatorLayer.lock()->userValue.v_int;
  ...
  void IRDocument::crateDotFile(std::ostream & dot) const {
    ...
    for (auto &kvp : _edges) {
      dot << "\t\"layer_" << kvp.from.lid << "\":p" << kvp.from.pid
          << " -> \"layer_" << kvp.to.lid << "\":p" << kvp.to.pid << " [];"
          << std::endl;
    }
    dot << "}" << std::endl;
  }

  InferenceEngine::InputInfo::Ptr IRDocument::createInput(
      const std::string &name, const TensorDims &dims) const {
    Layout layout;
    if (dims.size() == 4)
      layout = NCHW;
    ...
    std::cout << "createInput input data dims[0] " << dims[0] << "dims[1]"
              << dims[1] << std::endl;
...
#ifdef NNLOG
    ALOGI("createInput input info dims size %d and layout %d",
          info->getDims().size(), info->getLayout());
#endif

    auto indatadims = inputData->getDims();
    std::cout << " createInput input data indatadims[0] " << indatadims[0]
              << "indatadims[1]" << indatadims[1] << std::endl;
   ...
   * \param port
   */
  void IRDocument::saveOutputToIR(pugi::xml_node & parent,
                                  const DataPtr &port) {
    auto node = parent.append_child("port");
    node.append_attribute("id").set_value(port->userObject.v_int);
    // node.append_attribute("buffer").set_value(reinterpret_cast<size_t>(buffer)
    // & 0x00FFFFFF);
    if (!port->inputTo.empty()) {
      std::string comment = "connected to ";
      for (auto peer : port->inputTo)
        comment += ", " + peer.first;
      node.append_child(pugi::xml_node_type::node_comment)
          .set_value(comment.c_str());
    }
   ...
   * \param port
   */
  void IRDocument::saveInputToIR(pugi::xml_node & parent, int index,
                                 const DataPtr &port) {
    auto node = parent.append_child("port");
   ...
   * \param irLayer
   */
  void IRDocument::saveToIR(std::ostream & binFile, pugi::xml_node & parent,
    ...
    layer.append_attribute("precision")
        .set_value(IRBuilder::g_layer_precision == Precision::FP16 ? "FP16"
                                                                   : "FP32");

    if (!irLayer->params.empty()) {
      auto attr = layer.append_child(
          "data"); // todo: need to check for type and overide it
  ...
  void IRDocument::saveLayerToDot(std::ostream & dot, const IRLayer &irLayer)
      const {
    /*
    "layer_4" [
    ...
    */
    dot << "\t\"layer_" << irLayer->userValue.v_int << "\" [ label = \""
        << _name << "| type: " << irLayer->type;
    int pid = 0;
...
void IRDocument::saveBlobToIR(
  ...
  */
  ...
  // if(IRBuilder::g_layer_precision == Precision::FP16)  //fix me: handle float
  // case
  // const short* fp = blob->cbuffer().as<const short*>();
  // else
  const float *fp = blob->cbuffer().as<const float *>();
...
InferenceEngine::InputInfo::Ptr
IRDocument::createInput(const std::string &name, const TensorDims &dims) const {
  Layout layout;
  if (dims.size() == 4)
    layout = NCHW;
  // if (dims.size() == 4) layout = NHWC;
...
void IRDocument::saveLayerToDot(std::ostream &dot,
                                const IRLayer &irLayer) const {
  /*
  "layer_4" [
  label = "name| type | <f2> |-1"
  shape = "record"
