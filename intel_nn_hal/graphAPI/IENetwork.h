...
// ALOGI("outputInfo dims size = %d\n", outputDims.size());
  ...
  void setBlob(const std::string &inName, const Blob::Ptr &inputBlob) {
    ...
    ALOGI("Blob size %d and size in bytes %d bytes element size %d bytes",
          inputBlob->size(), inputBlob->byteSize(), inputBlob->element_size());
#endif
...
/*
        ...
        InferenceEngine::TBlob<PrecisionTrait<Precision::FP32>::value_type>::Ptr
   outputBlob;
        outputBlob =
