...
#define VLOGDIMS(l, d, header) \
    ...
    { \
        auto size = (d).size(); \
        VLOG(l, "%s: vectors {%d, %d, %d, %d}", header, (d)[0], \
             size > 1 ? (d)[1] : 0, size > 2 ? (d)[2] : 0, size > 3 ? (d)[3] :
0); \
    } while (0)
...
#define nnAssert(v)                                                            \
    ...
    {                                                                          \
        if (!(v))                                                              \
        {                                                                      \
            LOG(ERROR) << "nnAssert failed at " << __FILE__ << ":" << __LINE__ \
                       << " - '" << #v << "'\n";                               \
            abort();                                                           \
        }                                                                      \
    } while (0)
...
// This function convert f32 to f16 with rounding to nearest value to minimize
// error
...
short f32tof16(float x) {
  ...
  uint32_t s =
      (v.u >> 16) & 0x8000; // sign 16:  00000000 00000000 10000000 00000000
...
void f16tof32Arrays(float *dst, const short *src, uint32_t &nelem,
...
void f32tof16Arrays(short *dst, const float *src, uint32_t &nelem,
                    float scale = 1, float bias = 0) {
...
bool setRunTimePoolInfosFromHidlMemories(
...
static bool setInfoAndAllocateIfNeeded(RunTimeOperandInfo *info,
                                       const Shape &shape) {
  ...
  if (info->lifetime == OperandLifeTime::TEMPORARY_VARIABLE &&
      info->buffer == nullptr) {
...
// OutputPort handleFusion(const Model &model, const Operation &op, OutputPort
// &out, const int fusionIndex);
...
template <typename T>
T PreparedModel::ParseOperationInput(const Model &model,
...
T PreparedModel::GetConstFromBuffer(const uint8_t *buf, uint32_t len) {
  ...
  if (len != sizeof(T)) {
    ...
    // fix me if buffer is of type float and if float and
...
template <typename T>
std::vector<T> PreparedModel::GetConstVecFromBuffer(const uint8_t *buf,
                                                    uint32_t len) {
  int n = len / sizeof(T);
  if (n * sizeof(T) != len) {
...
const uint8_t *PreparedModel::GetOperandMemory(const Model &model,
                                               uint32_t index,
                                               uint32_t &len_out) {
  ...
  if (op.lifetime == OperandLifeTime::CONSTANT_COPY) {
    ...
    VLOG(L1, "operand is expected to be const, but lifetime is %d",
         op.lifetime);
    len_out = sizeOfData(op.type, op.dimensions);
    // nnAssert(false);
    return nullptr;
  ...
  nnAssert(false); // temp fix since some time const operand set as
                   // TEMPORARY_VARIABLE
  return nullptr;
}
...
template <typename T>
std::vector<T> PreparedModel::GetConstVecOperand(const Model &model,
                                                 uint32_t index) {
  dumpOperand(index);
  uint32_t len;
  const uint8_t *buf = GetOperandMemory(model, index, len);
...
Blob::Ptr PreparedModel::GetInOutOperandAsBlob(RunTimeOperandInfo &op,
                                               const uint8_t *buf,
                                               uint32_t &len) {
  return nullptr;
}
...
OutputPort PreparedModel::getPort(int index) {
  ...
  if (op.lifetime == OperandLifeTime::MODEL_INPUT) {
    ...
    auto operandInfo =
        mNet.createInput(operandName.str(), permuteDims(toDims(op.dimensions),
                                                        order)); // NHWC -> NCHW
    // auto operandInfo = mNet.createInput(operandName.str(),
    // toDims(op.dimensions)); // NHWC
    ...
    VLOG(L1, "mPorts[%d] %s dims size %d", index, mPorts[index]->name.c_str(),
         dims_size);

    auto dims = permuteDims(toDims(op.dimensions), order);
    // auto dims = toDims(op.dimensions);
    for (auto i = 0; i < dims.size(); i++)
      VLOG(L1, "input dims[%d] = %d & set input dims[%d] = %d ", i,
           op.dimensions[i], i, dims[i]);
  ...
  if (op.lifetime == OperandLifeTime::NO_VALUE) {
    VLOG(
        L1,
        "port is expected to be allocated for this as output from other layer");
    nnAssert(false);
  }
...
void PreparedModel::SetOperandMemory(const Model &model, uint32_t index,
                                     uint32_t &len_out, const uint8_t *buf) {}

/*
void PreparedModel::SetOperandFromTensor(uint8_t* buf, uint32_t &length,
IRBlob::Ptr infOutput)
{
...
*/
// TODO doublecheck
bool PreparedModel::validateRequest(const Request &request,
                                    const Model &model) {
  auto validRequestArguments = [&](const hidl_vec<RequestArgument> &arguments,
                                   ...
                                   size_t poolCount, const char *type) -> bool {
    const size_t argumentCount = arguments.size();
    if (argumentCount != operandIndexes.size()) {
      ALOGE("Request specifies %zu, but %s has the model as %zu", argumentCount,
            type, operandIndexes.size());
      return false;
    }
    for (size_t argumentIndex = 0; argumentIndex < argumentCount;
         argumentIndex++) {
      ...
      if (argument.hasNoValue) {
        if (argument.location.poolIndex != 0 || argument.location.offset != 0 ||
          ...
          ALOGE("Request %s: %zu has no value yet has details.", type,
                argumentIndex);
          return false;
        }
      }
      if (argument.location.poolIndex >= poolCount) {
        ALOGE("Request %s: %zu has an invalid poolIndex %d", type,
              argumentIndex, argument.location.poolIndex);
        return false;
      }
      ...
      if (rank > 0) {
        if (rank != operand.dimensions.size()) {
          ALOGE("Request %s: %zu  has number of dimensions (%d) different than "
                "the model's (%zu)",
        ...
        for (size_t i = 0; i < rank; i++) {
          if (argument.dimensions[i] != operand.dimensions[i] &&
            ...
            ALOGE("Request %s: %zu has dimension %zu of %d different than the "
                  "model's %d",
                  type, argumentIndex, i, operand.dimensions[i],
                  operand.dimensions[i]);
            return false;
          }
          if (argument.dimensions[i] == 0) {
            ALOGE("Request %s: %zu has dimension %zu of zero", type,
                  argumentIndex, i);
            return false;
          }
        }
  ...
  return (validRequestArguments(request.inputs, model.inputIndexes,
                                model.operands, pool_counts, "input") &&
          validRequestArguments(request.outputs, model.outputIndexes,
                                model.operands, pool_counts, "output"));
}

bool PreparedModel::validModel(const Model &model) {
  auto validOperandIndexes = [&](const hidl_vec<uint32_t> indexes,
                                 size_t operandCount) -> bool {
  ...
  for (size_t i = 0; i < operand_counts; i++) {
    ...
    case OperandLifeTime::CONSTANT_COPY:
      if (operand.location.offset + operand.location.length >
        ...
        ALOGE("OperandValue location out of range.  Starts at %d length %d. "
              "max %zu",
    ...
    case OperandLifeTime::CONSTANT_REFERENCE: {
      ...
      if (operand.location.poolIndex >= pool_counts) {
        ALOGE("Invalid poolIndex %d/%lu", operand.location.poolIndex,
              pool_counts);
        return false;
      }
      break;
...
bool PreparedModel::initializeRunTimeOperandInfo() {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    case OperandType::TENSOR_INT32:
      ...
      VLOG(L1, "OperandType::TENSOR_INT32 and operand scale value = %.1f",
           to.scale);
      break;
    case OperandType::TENSOR_QUANT8_ASYMM:
    ...
    case OperandLifeTime::CONSTANT_COPY:
      to.buffer =
          const_cast<uint8_t *>(&mModel.operandValues[from.location.offset]);
      to.numberOfUsesLeft = 0;
      break;
    case OperandLifeTime::CONSTANT_REFERENCE: {
...
void printOperandbuf(int level, const uint8_t *buffer,
                     const std::vector<uint32_t> &dims, int limit = 0) {
...
void PreparedModel::asyncExecute(const Request &request,
  ...
  auto inOutData = [this, &requestPoolInfos](
      const std::vector<uint32_t> &indexes,
      const hidl_vec<RequestArgument> &arguments, bool inputFromRequest,
      ExecuteNetwork *enginePtr, std::vector<OutputPort> mPorts) {
    ...
    for (size_t i = 0; i < indexes.size(); i++) {
      ...
      operand.length =
          arg.location.length; // sizeOfData(operand.type, operand.dimensions);

      VLOG(L1, "Copy request input/output to model input/output");
      // std::ostringstream operandName; operandName << "operand."<<indexes[i];
      // //use mPort[i]->name
      if (inputFromRequest) {
        // model/request oputput pointer pass to inference engine input
        // memcpy(operand.buffer, r.buffer + arg.location.offset,
        // operand.length)
        ...
        // memcpy(r.buffer + arg.location.offset, operand.buffer,
        // operand.length);
        /*
                auto outputDims =
   mPorts[indexes[i]]->getTensorDesc().getDims();
                auto nelem = sizeOf(outputDims);
                auto lenght = 4*nelem;
                uint8_t* tmpbuffer = new uint8_t[lenght];
                TensorDesc td(InferenceEngine::Precision::FP32,
   outputDims, Layout::ANY); //nhwc

                InferenceEngine::TBlob<float>::Ptr outputBlob =
   InferenceEngine::make_shared_blob<float>(td, (float *)tmpbuffer,
   lenght);
*/
...
*/
    ...
    /* //commented to run for MobileNet model and this only applicable for
single cts opeation test
for(const auto& op : mModel.operations) {
    const auto& o = mOperands[op.outputs[0]];
    InferenceEngine::TBlob<float>::Ptr opBlob =
enginePtr->getBlob(mPorts[op.outputs[0]]->name);
    VLOG(L1, "Operation %d has output 0(lifetime %d) are:", op.type,
o.lifetime);

    nelem = (opBlob->size() > 10 ? 10 : opBlob->size());
    for (int i = 0; i <  nelem; i++) {
    VLOG(L1, "operation output Blob elements %d = %f", i,
opBlob->readOnly()[i]);
    }
  ...
  if (!returned.isOk()) {
    ALOGE("hidl callback failed to return properly: %s",
          returned.description().c_str());
  }
}

Return<ErrorStatus>
PreparedModel::execute(const Request &request,
                       const sp<IExecutionCallback> &callback) {
...
bool PreparedModel::isOperationSupported(const Operation &operation,
                                         const Model &model) {
...
*/
  ...
  const auto &inputn =
      model.operands[operation.inputs[operation.inputs.size() - 1]];
...
bool PreparedModel::operationAdd(const Operation &operation) {
  ...
  VLOG(
      ...
      operation.inputs[0],
      isIn0Const ? "isIn0Const" : mPorts[operation.inputs[0]]->name.c_str(),
      operation.inputs[1],
 ...
 */
      ...
      calculateExplicitPadding(input_width, stride_width, filter_width,
                               pad_type /*padding_implicit*/, &padding_left,
                               &padding_right);
      calculateExplicitPadding(input_height, stride_height, filter_height,
                               pad_type /*padding_implicit*/, &padding_top,
                               &padding_bottom);
 ...
 * 9: An INT32 value, and has to be one of the {@link FusedActivationFunc}
 * values.
...
bool PreparedModel::operationConCat(const Operation &operation) {
  ...
  /*
 * Inputs:
 * 0 ~ n-1: The list on n input tensors, of shape [D0, D1, ..., Daxis(i), ...,
 * Dm]
 * n: An INT32 value, specifying the concatenation axis.
 * n+1: An INT32 value, and has to be one of the {@link FusedActivationFunc}
 * values.
 *    Specifies the activation to invoke on the result of each addition.
 */
...
***/
  ...
  // prms.weights = static_cast<IRBlob::Ptr>(filter); // permute OHWI to OIHW
  // (0->0, 3->1, 1->2, 2->3)
 ...
 */
  ...
  /*
   ...
   (uint32_t)filter_height, (uint32_t)filter_width};
    ...
    TensorDesc td(IRBuilder::g_layer_precision, newDims, {{newDims[2],
   newDims[3], newDims[0], newDims[1]}, {2, 3, 0, 1}}); //working for CTS

    //check diff combination btw TF lite and IE
    //2310
    //TensorDesc td(IRBuilder::g_layer_precision, newDims, {{newDims[2],
   newDims[3], newDims[1], newDims[0]}, {2, 3, 1, 0}});

    //using data_type = typename
   InferenceEngine::PrecisionTrait<IRBuilder::g_layer_precision>::value_type;
   //fix this to use calculate data type at runtime
   ...
   filter->cbuffer().as<const float*>()[filter->getTensorDesc().offset(i)];
   //set Layout::NCHW in td for src and dst
      }
   ...
   filter->cbuffer().as<const short*>()[filter->getTensorDesc().offset(i)];
   //set Layout::NCHW in td for src and dst
      }
 ...
 */
  ...
  auto input = getPort(operation.inputs[0]);
  // auto filter = GetConstOperandAsTensor(operation.inputs[1]); //NCHW [1,
  // depth_out, filter_height, filter_width]
  ...
  if (operation.inputs.size() == 11) {
    ...
    prms.num_output_planes =
        in_channels *
        depth_multiplier; // same as filter_out; //dims[0]; //depth out
 ...
 */
    ...
    prms.num_output_planes =
        in_channels * depth_multiplier; // same as filter_out;//depth out
  }

  /*
TF filter: 4-D with shape [filter_height, filter_width, in_channels,
channel_multiplier].
reshape to org layout of TF [filter_height, filter_width, in_channels,
channel_multiplier]
...
in_channels*depth_multiplier and
permute weights to (in_chennels, channel_multiplier, filter_height,
filter_width) where assuming TF original input filter
shape as [filter_height, filter_width,  in_channels, channel_multiplier] to
preapare in the layout expected by IE
*/
  ...
  (uint32_t)filter_height, (uint32_t)filter_width}; //channel_multiplier ==
  depth_multiplier working for CTS
  //TensorDims newDims = {(uint32_t)depth_multiplier, (uint32_t)in_channels,
  (uint32_t)filter_height, (uint32_t)filter_width};
  //TensorDims newDims = {1, in_channels*depth_multiplier, filter_height,
  filter_width}; //original filter shape //channel_multiplier ==
  depth_multiplier

  //TensorDesc td(IRBuilder::g_layer_precision, newDims, {{newDims[2],
  newDims[3], newDims[0], newDims[1]}, {2, 3, 0, 1}}); //working for CTS
  TensorDesc td(IRBuilder::g_layer_precision, newDims, {{newDims[1],
  newDims[0], newDims[2], newDims[3]}, {1, 0, 2, 3}});
  //TensorDesc td(InferenceEngine::Precision::FP16, newDims, {{newDims[3],
  newDims[2], newDims[1], newDims[0]}, {3, 2, 1, 0}});
...
*/
  ...
  VLOG(L1, "batches %d, channels %d, input_height: %d, input_width %d", batches,
 ...
 */
  ...
  if (inputDims.size() > 2) {
    ...
    TensorDims outDims = {
        (uint32_t)-1,
        numInputElements /
            batch_size}; // fix me: find correct outDims and if -1 is fine
    ...
    if (strechDim >= 0) {
      ...
      VLOG(L1, "numInputElements = %d, index = %d, outDims[index] = %d",
           numInputElements, strechDim, outDims[strechDim]);
    }
    ...
    /*
        ...
        if (precision == InferenceEngine::Precision::FP16) {
          TensorDesc td(InferenceEngine::Precision::FP16, newDims,
   {{newDims[0], newDims[1]}, {0, 1}});
   ...
   = input->cbuffer().as<const short*>()[filter->getTensorDesc().offset(i)];
   //set Layout::NCHW in td for src and dst
              }
          }
        }
        else if (precision == InferenceEngine::Precision::FP32) {
          TensorDesc td(InferenceEngine::Precision::FP32, newDims,
   {{newDims[0], newDims[1]}, {0, 1}});
   ...
   = input->cbuffer().as<const float*>()[filter->getTensorDesc().offset(i)];
   //set Layout::NCHW in td for src and dst
              }
          }
        }
...
*/
  ...
  mPorts[operation.outputs[0]] = L2Normalization(
      getPort(operation.inputs[0]), false, false); // passing accross false
  return true;
}
 ...
 */
  ...
  float k = PARAM_FP(2);
  // mPorts[operation.outputs[0]] = LRN(getPort(operation.inputs[0]), alpha,
  // beta, size, true, k);
  mPorts[operation.outputs[0]] =
      LRN(getPort(operation.inputs[0]), alpha, beta, size, false, k);
...
{
    VLOG("operation type LSTM is supported, but not yet in this
implementation");
    nnAssert(true);
...
bool PreparedModel::operationMUL(const Operation &operation) {
  mPorts[operation.outputs[0]] =
      handleFusion(getPort(operation.inputs[0]) * getPort(operation.inputs[1]),
                   PARAM_I32(2));
  return true;
}
 ...
 * 1: A 1-D tensor of type {@link OperandType::TENSOR_INT32}, defining the
 * shape
 *    of the output tensor. The number of elements implied by shape must be
 * the same
 ...
 */
  /* note: todo: We need to be careful here, inter-tensors are in different
 * order,
 ...
 */
  ...
  auto outDims =
      toDims(GetConstVecOperand<uint32_t>(mModel, operation.inputs[1]));
  ...
  // auto numInputElements = getNumberOfElements(static_cast<const
  // vec<uint32_t>> (inDims));
  ...
  auto numOutputElements = 1; // shape
  for (auto i = 0; i < outDims.size(); i++) {
    VLOG(L1, "operand1: shape of output tensor outDims[%d] = %d ", i,
         outDims[i]);
    if ((int)outDims[i] < 0) {
      strechDim = i; // strechdim
      VLOG(L1, "strechDim = %d", i);
  ...
  if (strechDim >= 0) {
    ...
    VLOG(L1, "numInputElements or size = %d, index = %d, outDims[index] = %d",
         numInputElements, strechDim, outDims[strechDim]);
  }
  ...
  if (numInputElements != numOutputElements) {
    VLOG(L1, "numInputElements is not equal to numOutputElements",
         numInputElements, numOutputElements);
    nnAssert(false);
  }
  // Note: " error [VPU] Unsupported 1 D dimensions" for reshape output and fix
  // me
 ...
 */
  ...
  /*
    ...
    if (inputDims.size() == 2) {
        uint32_t batch_size = inputDims[0];//getSizeOfDimension(inputShape,
   0);
        uint32_t input_size = sizeOf(inputDims) / batch_size;
   //getNumberOfElements(inputShape) / batch_size;
...
*/
    ...
    VLOG(L1, "mPorts[%d] %s dims size %d", i, mPorts[i]->name.c_str(),
         dims_size);
...
VLOG(L1, "output dims[%d] = %d & set output dims[%d] = %d ", j,
mOperands[i].dimensions[j], j, outputDims[j]);
...
*/
    ...
    if (nelem != inputElem) {
      ...
      mOperands[i].length =
          sizeOfData(mOperands[i].type, mOperands[i].dimensions);
    }
  }
}
...
*/
    ...
    if (nelem != outputElem) {
      VLOG(L1, "set correct dims as operand output dims different than real "
               "output dims\n");
      /*
for (auto j = 0; j < outputDims.size(); j++)
mOperands[i].dimensions[j] = static_cast<uint32_t>(outputDims[j]);
mOperands[i].length = sizeOfData(mOperands[i].type,
mOperands[i].dimensions);
*/
    }
  }
}
...
IRBlob::Ptr VpuPreparedModel::GetConstWeightsOperandAsTensor(uint32_t index) {
  ...
  if (op.type == OperandType::TENSOR_FLOAT32 ||
    ...
    VLOGDIMS(L1, permuteDims(toDims(op.dimensions), order),
             "weights/bias dims");
    VLOG(L1, "Model buffer oplength = %d bytes nelem= %d fp16Array_length= %d "
    ...
    if (inputDims.size() != 4) {
      // InferenceEngine::TBlob<short>::Ptr blob =
      // std::make_shared<InferenceEngine::TBlob<short>>(td,(short
      // *)inputFilter_fp16, fp16Array_length);
      return blob;
    } else {
      ...
      size_t offset =
          0; // blob->size() == o*i*h*w and simlar to nchw memory layout
...
// todo: create a readOnly blob that accepts const pointers
// return std::make_shared<InferenceEngine::TBlob<float>>(td, (float *)buf,
// len);
#endif
  } else if (op.type == OperandType::TENSOR_INT32) {

    VLOG(L1, "check if const tensors of type IN32 supported");
    TensorDesc td(InferenceEngine::Precision::I32, toDims(op.dimensions),
                  Layout::ANY);
    if (buf == nullptr) {
...
IRBlob::Ptr VpuPreparedModel::GetConstOperandAsTensor(uint32_t index) {
  ...
  VLOG(L1, "VpuPreparedModel:: Operand: index: %d, len: %d, buf: %p", index,
       len, buf);
  if (op.type == OperandType::TENSOR_FLOAT32 ||
      op.type == OperandType::FLOAT32) {
#ifndef MYRIAD_FP32 // Myriad only supprts FP16
...
Blob::Ptr VpuPreparedModel::GetInOutOperandAsBlob(RunTimeOperandInfo &op,
                                                  const uint8_t *buf,
                                                  uint32_t &len) {
  // const auto op = model.operands[index];
  // uint32_t len;
  ...
  if (op.type == OperandType::TENSOR_FLOAT32 ||
            ...
            for (size_t i = 0; i < in_depth; i++) {
              for (size_t h = 0; h < height; h++) {
                for (size_t w = 0; w < width; w++) {
                  ...
                  // blob->buffer().as<float*>()[blob->getTensorDesc().offset(offset++)]
                  // = input[offset_nhwc];
                  // size_t offset_nchw = b*in_depth*height*width +
                  // i*height*width + h*width + w; //similar to NCHW memory
                  // layout
                  // blob->buffer().as<float*>()[offset_oihw] =
                  // inputFilter[offset_ohwi];
                  // VLOG(L1, "offset_nhwc= %d offset_nchw= %d", offset_nhwc,
                  // offset_nchw);
                }
              }
            }
    ...
    if (op.lifetime == OperandLifeTime::MODEL_INPUT) {
      ...
      // InferenceEngine::TBlob<short>::Ptr blob =
      // InferenceEngine::make_shared_blob<short,
      ...
      uint32_t nelem = getNumberOfElements(op.dimensions);
      VLOG(L1, "Model buffer oplength = %d bytes nelem= %d fp16Array= %d bytes "
      ...
      TensorDesc td(InferenceEngine::Precision::FP16, toDims(op.dimensions),
                    layout);
      // todo: create a readOnly blob that accepts const pointers
      InferenceEngine::TBlob<short>::Ptr blob =
          std::make_shared<InferenceEngine::TBlob<short>>(td);
      ...
      VLOG(L1,
           "Model buffer len = %d bytes length= %d bytes fp16Array= %d bytes\n",
           len, length, sizeof(fp16Array));
      if (length >= sizeof(fp16Array)) {
        VLOG(L1, "Model buffer len = %d bytes length= %d bytes fp16Array= %d "
                 "bytes\n",
             len, length, sizeof(fp16Array));
        nnAssert(false);
      }
...
// len);
    ...
    // TensorDesc td(InferenceEngine::Precision::I32, toDims(op.dimensions),
    // Layout::ANY);
    // return std::make_shared<InferenceEngine::TBlob<int32_t>>(td, (int32_t
    // *)buf, len);
  } else {
...
IRBlob::Ptr CpuPreparedModel::GetConstWeightsOperandAsTensor(uint32_t index) {
  ...
  uint32_t len;
  const uint8_t *buf = GetOperandMemory(mModel, index, len);
  VLOG(L1, "CpuPreparedModel:: Operand: index: %d, len: %d, buf: %p", index,
       len, buf);
  if (op.type == OperandType::TENSOR_FLOAT32 ||
      op.type == OperandType::FLOAT32) {
    vec<unsigned int> order;
...
Blob::Ptr CpuPreparedModel::GetInOutOperandAsBlob(RunTimeOperandInfo &op,
                                                  const uint8_t *buf,
                                                  uint32_t &len) {
  if (op.type == OperandType::TENSOR_FLOAT32 ||
      op.type == OperandType::FLOAT32) {
    ...
    TensorDesc td(InferenceEngine::Precision::I32, toDims(op.dimensions),
                  Layout::ANY);
    return std::make_shared<InferenceEngine::TBlob<int32_t>>(td, (int32_t *)buf,
                                                             len);
  } else {
    VLOG(L1, "not supporting const tensors of type ", op.type);
    nnAssert(false);
...
} // namespace android} else if (op.type == OperandType::TENSOR_INT32) {
VLOG(L1, "check if const tensors of type IN32 supported");
// nnAssert(true);
TensorDesc td(InferenceEngine::Precision::I32, toDims(op.dimensions),
              Layout::ANY);
