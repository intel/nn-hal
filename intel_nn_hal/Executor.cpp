...
#define VLOGDIMS(l, d, header)                                                 \
  do {                                                                         \
    ...
    VLOG(l, "%s: vectors {%d, %d, %d, %d}", header, (d)[0],                    \
         size > 1 ? (d)[1] : 0, size > 2 ? (d)[2] : 0, size > 3 ? (d)[3] : 0); \
  } while (0)
...
template <typename T>
T Executor::ParseOperationInput(const Model *model, const Operation &operation,
                                uint32_t index) {
...
template <typename T>
std::vector<T> Executor::GetConstVecFromBuffer(const uint8_t *buf,
                                               uint32_t len) {
  int n = len / sizeof(T);
  if (n * sizeof(T) != len) {
...
const uint8_t *Executor::GetOperandMemory(const Model *model, uint32_t index,
                                          uint32_t &len_out) {
...
template <typename T>
std::vector<T> Executor::GetConstVecOperand(const Model *model,
                                            uint32_t index) {
  dumpOperand(index);
  uint32_t len;
  const uint8_t *buf = GetOperandMemory(model, index, len);
...
Blob::Ptr Executor::GetInOutOperandAsBlob(RunTimeOperandInfo &op,
...
void Executor::SetOperandMemory(const Model *model, uint32_t index,
                                uint32_t &len_out, const uint8_t *buf) {}

bool Executor::initializeRunTimeInfo(
  ...
  auto updateForArguments =
      [this, &requestPoolInfos](const std::vector<uint32_t> &indexes,
...
IRBlob::Ptr VpuExecutor::GetConstWeightsOperandAsTensor(uint32_t index) {
  ...
  const auto op = mOperands[index];
  VLOG(L1, "VpuExecutor:: Operand: index: %d, len: %d, buf: %p", index, len,
       buf);
...
Blob::Ptr VpuExecutor::GetInOutOperandAsBlob(RunTimeOperandInfo &op,
                                             const uint8_t *buf,
                                             uint32_t &len) {
  // const auto op = model.operands[index];
  // uint32_t len;
...
IRBlob::Ptr CpuExecutor::GetConstWeightsOperandAsTensor(uint32_t index) {
  ...
  uint32_t len;
  const uint8_t *buf = GetOperandMemory(mModel, index, len);
  VLOG(L1, "CpuExecutor:: Operand: index: %d, len: %d, buf: %p", index, len,
       buf);
  if (op.type == OperandType::TENSOR_FLOAT32 ||
      op.type == OperandType::FLOAT32) {
    vec<unsigned int> order;
...
Blob::Ptr CpuExecutor::GetInOutOperandAsBlob(RunTimeOperandInfo &op,
                                             const uint8_t *buf,
                                             uint32_t &len) {
  if (op.type == OperandType::TENSOR_FLOAT32 ||
      op.type == OperandType::FLOAT32) {
...
void PreparedModel::asyncExecute(const Request &request,
  ...
  std::vector<RunTimePoolInfo> requestPoolInfos;
  if (!executor::setRunTimePoolInfosFromHidlMemories(&requestPoolInfos,
                                                     request.pools)) {
...
bool PreparedModel::initialize() {
  return executor::setRunTimePoolInfosFromHidlMemories(&mPoolInfos,
                                                       mModel.pools);
}
