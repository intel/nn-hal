...
#undef LOG_TAG
#define LOG_TAG "UnidirectionalSequenceLSTM"
...
UnidirectionalSequenceLSTM::UnidirectionalSequenceLSTM(int operationIndex)
    : OperationsBase(operationIndex) {
    mDefaultOutputIndex = sModelInfo->getOperationOutput(mNnapiOperationIndex, 0);
}

bool UnidirectionalSequenceLSTM::validate() {
...
std::shared_ptr<ngraph::Node> UnidirectionalSequenceLSTM::createNode() {
    ...
    if (inputsSize == 28) {
        ...
        if (sModelInfo->isOmittedInput(mNnapiOperationIndex, 24) &&
            ...
            if (isCIFGenabled) {
                if (isValidInputTensor(25) && isValidInputTensor(26) && isValidInputTensor(27))
    ...
    auto isTimeMajor = sModelInfo->ParseOperationInput<uint8_t>(mNnapiOperationIndex, 23);
    ...
    if (isLayerNormUsed) {
        if (!isCIFGenabled) input_layer_norm_weights = getInputNode(24);
        forget_layer_norm_weights = getInputNode(25);
        cell_layer_norm_weights = getInputNode(26);
        output_layer_norm_weights = getInputNode(27);
    }
    ...
    for (uint32_t i = 0; i < maxTime; i++) {
        ...
        if (!isCIFGenabled)
            i_t = add(add(matMul(inputSplit[i], input2input_weights, false, true),
        ...
        f_t = add(add(matMul(inputSplit[i], input2forget_weights, false, true),
        ...
        c_t = add(matMul(inputSplit[i], input2cell_weights, false, true),
                  matMul(initial_hidden_state, recurrent2cell_weights, false, true));
        // o_t = W_{xo}x_t+W_{ho}h_{t-1}
        o_t = add(matMul(inputSplit[i], input2output_weights, false, true),
        ...
        output_at_each_timestep.push_back(H);
    }
...
std::shared_ptr<ngraph::Node> UnidirectionalSequenceLSTM::add(
...
std::shared_ptr<ngraph::Node> UnidirectionalSequenceLSTM::applyActivation(
...
std::shared_ptr<ngraph::Node> UnidirectionalSequenceLSTM::LayerNorm(
...
bool UnidirectionalSequenceLSTM::isValidInputTensor(uint32_t inputIndex) {
